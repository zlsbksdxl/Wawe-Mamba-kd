...
class SAM(nn.Module):
    ...
    def forward(self, x, x_img):
        #print(f"SAM input x shape: {x.shape}, x_img shape: {x_img.shape}")
        '''
        # Ensure that input 'x' has the correct shape
        if x_img.shape[1] != x.shape[1]:
            x_img = x_img.permute(0, 3, 1, 2).contiguous()
            if x_img.shape[1] != x.shape[1]:
                raise ValueError(
                    f"x_img channels {x_img.shape[1]} do not match x channels {x.shape[1]}"
                )
        '''
        # First part: Generate attention-guided features
        x1 = self.conv1(x)
        ...
        #x1 = x1 * x2  # Attention applied to feature map
        #x1 = x1 + x  # Add back to the identity path

        # Second part: Calculate PSNR on patches
        # Split the de-rained image into 4 patches
        patches = self.split_into_patches(img)
        #print(f'x_img shpae:{x_img.shape}')
        # Calculate PSNR for each patch with the original input
        psnr_values = self.calculate_psnr(patches, x_img)

        # Apply PSNR-based attention to SS2D sequence
        attention_weights = self.apply_psnr_attention(psnr_values)

        return x1, img, attention_weights

    def split_into_patches(self, img):
        """Split the image into 4 patches (assuming H x W is divisible by 2)"""
        b, c, h, w = img.shape
        if h % 2 != 0 or w % 2 != 0:
            raise ValueError(f"Image size ({h}, {w}) must be divisible by 2 for patch splitting.")
    ...
    '''
    def calculate_psnr(self, patches, original_img):
        """Calculate PSNR for each patch against the original image"""
        psnr_values = []
        for patch in patches:
            # Compute PSNR for each patch
            h, w = patch.shape[2:]
            target_patch = original_img[:, :, :h, :w]

            mse = F.mse_loss(patch, target_patch, reduction='mean')
            psnr = 10 * torch.log10(1.0 / mse)  # Assuming pixel values are in the range [0, 1]
            psnr_values.append(psnr)
        return psnr_values
    '''
    def calculate_psnr(self, patches, original_img):
        """为批次中的每个样本计算每个 patch 与原始图像的 PSNR 值"""
        b = original_img.shape[0]  # 获取批次大小
        psnr_values = []
        for i in range(b):
            patch_psnr = []
            for patch in patches:
                # 为当前样本的每个 patch 计算 PSNR
                h, w = patch.shape[2:]  # 获取 patch 的高度和宽度
                target_patch = original_img[i:i+1, :, :h, :w]  # 提取对应区域
                mse = F.mse_loss(patch[i:i+1], target_patch, reduction='mean')  # 计算均方误差
                psnr = 10 * torch.log10(1.0 / mse.clamp(min=1e-10))  # 计算 PSNR，避免除以零
                patch_psnr.append(psnr)
            psnr_values.append(torch.stack(patch_psnr))
        return torch.stack(psnr_values)  # 返回形状为 [B, 4] 的张量

    '''
    def apply_psnr_attention(self, psnr_values):
        """Generate attention weights based on PSNR values"""
        attention_weights = torch.softmax(torch.tensor(psnr_values), dim=0)
        return attention_weights
    '''
    def apply_psnr_attention(self, psnr_values):
        """基于 PSNR 值生成批次的注意力权重"""
        attention_weights = torch.softmax(psnr_values, dim=1)  # 在 patch 维度上应用 softmax
        return attention_weights  # 返回形状为 [B, 4] 的张量
...
class SS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor, attention_weights: torch.Tensor):
        B, C, H, W = x.shape
        L = H * W
        K = 4
        ...
        #print(f'dts shape:{dts.shape}, attention_weights shpae:{attention_weights.shape if attention_weights!=None else None}')

        # Apply the attention weights to the sequence, if attention_weights are not None
        if attention_weights is not None:
            attention_weights = attention_weights.view(B, 4, 1, 1).to(dts.device)  # Rescale the sequence of features using attention weights
            #print(f'attention_weights shape:{attention_weights.shape}')
            dts = dts * attention_weights
    ...
    def forward(self, x: torch.Tensor, attention_weights: torch.Tensor, **kwargs):
        B, H, W, C = x.shape
        ...
        y1, y2, y3, y4 = self.forward_core(x, attention_weights)
...
class LFSSBlock(nn.Module):
    def __init__(
            ...
            d_state: int = 16,
            expand: float = 2.,
            sam_module: nn.Module = None,  # Add SAM module as parameter
            **kwargs,
    ):
        super().__init__()
        ...
        # Initialize the SAM module if provided
        self.sam = sam_module if sam_module is not None else SAM(n_feat=hidden_dim, kernel_size=3, bias=True)


    def forward(self, input, x_size, output=None, input_img=None):
        # x [B,HW,C]
        B, L, C = input.shape
        ...
        # Apply SAM module if previous layer outputs are available
        #print(f'output shape in LFSSBlock:{output.shape}, input_img shape in LFSSBlock:{input_img.shape}')
        if output is not None and input_img is not None:
            x_sam, img_rs, attention_weights = self.sam(output, input_img)
        else:
            attention_weights = None

        x = input*self.skip_scale + self.drop_path(self.self_attention(x, attention_weights))  # Using SAM output and attention weights
...
class WaveletPooling(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # 用于融合低频和高频子带的卷积层
        self.fusion_conv = nn.Conv2d(in_channels*2, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
        
        # 用于保持空间维度一致的卷积层
        self.pool = nn.Conv2d(out_channels, out_channels, 1, 1, 0, bias=False)  # 保持空间尺寸一致
    
    def forward(self, x_LL, x_h):
        """
        x_LL: 低频子带，shape: (batch_size, in_channels, h, w)
        x_h: 高频子带，shape: (batch_size, in_channels, h, w)
        """
        # 将低频和高频子带拼接在一起
        x_fused = torch.cat([x_LL, x_h], dim=1)  # 拼接后的形状为 (batch_size, in_channels*2, h, w)
        
        # 使用1x1卷积融合高低频子带
        x_fused = self.fusion_conv(x_fused)  # 融合后的形状为 (batch_size, out_channels, h, w)
        
        # 使用1x1卷积保持输出的空间尺寸一致
        x_out = self.pool(x_fused)  # 输出形状为 (batch_size, out_channels, h, w)
...
class MSFN(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias=False):
        ...
        # 根据拼接后的通道数，修改输入通道数为 64
        hidden_features = int(dim * ffn_expansion_factor)
        self.project_in = nn.Conv2d(64, hidden_features * 2, kernel_size=1, bias=bias)  # 修改为 64
    ...
    def forward(self, x_LL, x_h):
        # 通道扩展
        x = torch.cat([x_LL, x_h], dim=1)  # 拼接低频和高频子带
        x = self.project_in(x)
...
class DownFRG(nn.Module):
    def __init__(self, dim, n_l_blocks=1, n_h_blocks=1, expand=2):
        ...
        #self.wavelet_pooling  = WaveletPooling(in_channels=64, out_channels=64)
        self.msfm = MSFN(dim=dim, ffn_expansion_factor=expand)
    
    def forward(self, x, x_d, prev_output=None, prev_x_d=None):
        x_LL, x_HL, x_LH, x_HH = self.dwt(x)
        b, c, h, w = x_LL.shape
        x_h = self.h_fusion([x_HL, x_LH, x_HH])
        x_h = self.l_conv(torch.cat([x_h, x_d], dim=1))
        x_h = rearrange(x_h, "b c h w -> b (h w) c").contiguous()
        for l_layer in self.l_blk:
            x_h = l_layer(x_h, [h, w], prev_output, prev_x_d)
        x_h = rearrange(x_h, "b (h w) c -> b c h w", h=h, w=w).contiguous()
        
        for h_layer in self.h_blk:
            x_LL = h_layer(x_LL, x_h)
            
        #print(f'x_LL shape:{x_LL.shape}, x_h shape:{x_h.shape}')
        output = self.msfm(x_LL, x_h)
        
        #print(f'down shape:\nx_LL shape:{x_LL.shape},x_HL shape:{x_HL.shape},x_LH shape:{x_LH.shape},x_HH shape:{x_HH.shape}')
        return output, x_LL, x_h
...
class upFRG(nn.Module):
    ...
    def forward(self, x_l, x_h, prev_output=None, prev_x_d=None):
        b, c, h, w = x_l.shape
        x_h = rearrange(x_h, "b c h w -> b (h w) c").contiguous()
        for l_layer in self.l_blk:
            x_h = l_layer(x_h, [h, w], prev_output, prev_x_d)
        ...
        #print(f'up shape:x_LL shape:{x_l.shape},x_HL shape:{x_HL.shape},x_LH shape:{x_LH.shape},x_HH shape:{x_HH.shape}')
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        ######## UNet-64 ########
        # Down-path (Encoder)
        output, _, x_h1 = self.down_group1(x1, img_down1)
        output, _, x_h2 = self.down_group2(output, img_down2, output, img_down1)
        output, x_l, x_h3 = self.down_group3(output, img_down3, output, img_down2)

        # Up-path (Decoder)
        x_l = self.up_group3(x_l, x_h3, output, img_down3)
        x_l = self.up_group2(x_l, x_h2, x_l, img_down2)
        x_l = self.up_group1(x_l, x_h1, x_l, img_down1)
